...
MemberList::MemberMap AddressType::nativeMembers(ContractDefinition const *) const
{
  MemberList::MemberMap members = {
      ...
      {"tokenBalance", TypeProvider::function(strings{"vrcToken"}, strings{"uint"}, FunctionType::Kind::TokenBalance, false, StateMutability::View)},
  };
  if (m_stateMutability == StateMutability::Payable)
  {
    ...
    members.emplace_back(MemberList::Member{"transferToken", TypeProvider::function(strings{"uint", "vrcToken"}, strings(), FunctionType::Kind::TransferToken)});
  }
  return members;
}
...
TypeResult IntegerType::unaryOperatorResult(Token _operator) const
{
  ...
  // no further unary operators for vrcToken
  else if (isVrcToken())
    return TypeResult::err("");
  // for non-vrctoken integers
  // we allow -, ++ and --
...
tuple<bool, rational> RationalNumberType::isValidLiteral(Literal const &_literal)
{
  ...
  case Literal::SubDenomination::Vdt:
  ...
  case Literal::SubDenomination::Ether:
    ...
    break;
  case Literal::SubDenomination::Vs:
...
StringLiteralType::StringLiteralType(string const &_value) : m_value{_value}
{
}
...
ArrayType::ArrayType(DataLocation _location, bool _isString) : ReferenceType(_location),
...
FunctionType::FunctionType(FunctionTypeName const &_typeName) : m_parameterNames(_typeName.parameterTypes().size(), ""),
...
string FunctionType::richIdentifier() const
{
  ...
  case Kind::ValidateMultiSign:
    id += "validatemultisign";
    break;
  case Kind::BatchValidateSign:
    id += "batchvalidatesign";
    break;
  case Kind::SHA256:
    id += "sha256";
    break;
...
MemberList::MemberMap MagicType::nativeMembers(ContractDefinition const *) const
{
  ...
  {
  case Kind::Block:
    return MemberList::MemberMap({{"coinbase", TypeProvider::payableAddress()},
  ...
  case Kind::Message:
    return MemberList::MemberMap({
        ...
        {"tokenid", TypeProvider::vrcToken()},
    });
  case Kind::Transaction:
    return MemberList::MemberMap({{"origin", TypeProvider::payableAddress()},
                                  {"gasprice", TypeProvider::uint256()}});
  case Kind::ABI:
    return MemberList::MemberMap({{"encode", TypeProvider::function(
                                                 TypePointers{},
