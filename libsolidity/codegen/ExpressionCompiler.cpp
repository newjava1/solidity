...
bool ExpressionCompiler::visit(FunctionCall const& _functionCall)
{
	...
	else
	{
		...
		case FunctionType::Kind::Transfer:
			_functionCall.expression().accept(*this);
			// Provide the gas stipend manually at first because we may send zero vs.
			// Will be zeroed if we send more than zero vs.
...
)
{
	...
	// <stack top>
	// vrcToken [if _functionType.tokenSet()]
	...
	// [value,] addr, gas, vrcToken (stack top)
	if (isDelegateCall)
