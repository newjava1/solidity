...
CompilerStack::CompilerStack(ReadCallback::Callback const &_readFile) : m_readFile{_readFile},
																		m_generateIR{false},
																		m_errorList{},
...
bytes CompilerStack::createCBORMetadata(string const &_metadata, bool _experimentalMode)
{
	...
	encoder.pushBytes("vision", dev::swarmHash(_metadata).asBytes());
...
string CompilerStack::computeSourceMapping(eth::AssemblyItems const &_items) const
{
	...
	for (auto const &item : _items)
	{
		...
		int sourceIndex =
			location.source && sourceIndicesMap.count(location.source->name()) ? sourceIndicesMap.at(location.source->name()) : -1;
		char jump = '-';
