...
namespace dev
{
	namespace solidity
	{
		...
		Parser::FunctionHeaderParserResult Parser::parseFunctionHeader(bool _forceEmptyName, bool _allowModifiers)
		{
			...
			while (true)
			{
				...
				if (_allowModifiers && token == Token::Identifier)
				{
					...
					if ((result.name->empty() && !result.isConstructor) && (m_scanner->peekNextToken() == Token::Semicolon ||
		...
		{
			RecursionGuard recursionGuard(*this);
			ASTNodeFactory nodeFactory = _partiallyParsedExpression ? ASTNodeFactory(*this, _partiallyParsedExpression) : ASTNodeFactory(*this);
			Token token = m_scanner->currentToken();
		...
		ASTPointer<Expression> Parser::parsePrimaryExpression()
		{
			...
			case Token::Number:
				...
				else if (TokenTraits::isVisionSubdenomination(m_scanner->peekNextToken()))
				{
