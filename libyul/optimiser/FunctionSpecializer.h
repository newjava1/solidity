 ...
 *   }
 *
 * Further optimization steps will be able to simplify the function even further. The optimization
 * step is mainly useful for functions that would not be inlined.
 *
...
class FunctionSpecializer: public ASTModifier
{
	...
	using Arguments = std::vector<std::optional<Expression>>;
...
public:
	...
	virtual void operator()(FunctionCall& _f) override;
...
private:
	...
	/// `i`-th argument can be specialized.
	Arguments specializableArguments(FunctionCall const& _f);
	...
	///
	/// Note that the return function definition will have new (and unique) names, for both the
	/// function and variable declarations.
	///
	/// For example, if `_f` is the function `function f(a, b, c) -> d { sstore(a, b) }` and
	/// `_arguments` is the vector of literals `{1, 2}`. The returned function could be:
	///
	...
	std::map<YulString, std::vector<std::pair<YulString, Arguments>>> m_oldToNewMap;
