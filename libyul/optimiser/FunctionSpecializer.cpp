...
public:
	FunctionCopier(
		map<YulString, YulString> _translations
	):
...
FunctionSpecializer::Arguments FunctionSpecializer::specializableArguments(FunctionCall const& _f)
{
...
void FunctionSpecializer::operator()(FunctionCall& _f)
{
	...
	Arguments arguments = specializableArguments(_f);

	if (boost::algorithm::any_of(arguments, [](auto& _a) { return _a.has_value(); }))
	{
		auto newName = m_nameDispenser.newName(_f.functionName.name);
		yulAssert(newName != _f.functionName.name, "Name dispenser returned the same name.");
		m_oldToNewMap[_f.functionName.name].emplace_back(make_pair(newName, arguments));

		_f.functionName.name = newName;
		_f.arguments = unusedFunctionsCommon::filter(
			_f.arguments,
...
FunctionDefinition FunctionSpecializer::specialize(
	...
	YulString _newName,
	FunctionSpecializer::Arguments _arguments
)
{
	...
	// Function parameters that will be specialized inside the body is converted into a variable
	// declaration.
	vector<Statement> missingVariableDeclarations;
	for (size_t i = 0; i < _f.parameters.size(); ++i)
		if (_arguments[i])
			missingVariableDeclarations.emplace_back(
				VariableDeclaration{
					_f.location,
					vector<TypedName>{newFunction.parameters[i]},
					make_unique<Expression>(move(*_arguments[i]))
				}
			);
	...
	// TODO move filter into something common?
	newFunction.parameters =
		unusedFunctionsCommon::filter(
			newFunction.parameters,
...
void FunctionSpecializer::run(OptimiserStepContext& _context, Block& _ast)
{
	...
	iterateReplacing(_ast.statements, [&](Statement& _s) -> optional<vector<Statement>>
	{
		...
		{
			// TODO skip functions that are too simple, since they will get inlined anyway.
			// TODO maybe make the changes aggresively if we know that the function cannot be inlined.
