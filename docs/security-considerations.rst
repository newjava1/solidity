...
of Vs hands over control to that contract (B). This makes it possible for B
...
::
    ...
    contract Fund {
        /// Mapping of vs shares of the contract.
...
of ``send``, but it still exposes a weakness: Vs transfer can always
include code execution, so the recipient could be a contract that calls
back into ``withdraw``. This would let it get multiple refunds and
basically retrieve all the Vs in the contract. In particular, the
...
Note that re-entrancy is not only an effect of Vs transfer but of any
...
Sending and Receiving Vs
===========================

- Neither contracts nor "external accounts" are currently able to prevent that someone sends them Vs.
  Contracts can react on and reject a regular transfer, but there are ways
  to move Vs without creating a message call. One way is to simply "mine to"
  the contract address and the second way is using ``selfdestruct(x)``.

- If a contract receives Vs (without a function being called), the fallback function is executed.
  If it does not have a fallback function, the Vs will be rejected (by throwing an exception).
  ...
  To be sure that your contract can receive Vs in that way, check the gas requirements of the fallback function
...
- Use the most precise units to represent the vdt amount as possible, as you lose
  any that is rounded due to a lack of precision.

- If you want to send Vs using ``address.transfer``, there are certain details to be aware of:
  ...
  2. Sending Vs can fail due to the call depth going above 1024. Since the caller is in total control of the call
     ...
     write your contract using a pattern where the recipient can withdraw Vs instead.
  3. Sending Vs can also fail because the execution of the recipient contract
...
Now someone tricks you into sending vs to the address of this attack wallet:
...
Restrict the Amount of Vs
============================

Restrict the amount of Vs (or other tokens) that can be stored in a smart
contract. If your source code, the compiler or the platform has a bug, these
funds may be lost. If you want to limit your loss, limit the amount of Vs.
...
are the arguments in range, did they send enough Vs, does the person
...
self-checks like "Has any Vs leaked?",
