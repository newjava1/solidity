...
::
    ...
    contract C {
        ...
        function createAndEndowD(uint arg, uint amount) public payable {
            // Send vs along with the creation
...
#. If your contract receives Vs via a public function without ``payable`` modifier (including the constructor and the fallback function).
#. If your contract receives Vs via a public getter function.
#. If a ``.transfer()`` fails.
...
::
    ...
    contract VendingMachine {
        function buy(uint amount) public payable {
            if (amount > msg.value / 2 vs)
                revert("Not enough Vs provided.");
            // Alternative way to do it:
            require(
                amount <= msg.value / 2 vs,
                "Not enough Vs provided."
            );
