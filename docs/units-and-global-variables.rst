...
A literal number can take a suffix of ``vdt`` or ``vs`` to convert between the subdenominations of Vs, where Vs currency numbers without a postfix are assumed to be vdt, e.g. ``2 vs == 2000000 vdt`` evaluates to ``true``.
...
- ``msg.value`` (``uint``): number of vdt sent with the message
...
``<address>.balance`` (``uint256``):
    balance of the :ref:`address` in vdt
``<address payable>.transfer(uint256 amount)``:
    send given amount of vdt to :ref:`address`, reverts on failure, forwards 2300 gas stipend, not adjustable
``<address payable>.send(uint256 amount) returns (bool)``:
    send given amount of vdt to :ref:`address`, returns ``false`` on failure, forwards 2300 gas stipend, not adjustable
...
.. warning::
    ...
    to make safe Vs transfers, always check the return value of ``send``, use ``transfer`` or even better:
