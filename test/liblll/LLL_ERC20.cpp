...
(seq
  ...
  ;; --------------------------------------------------------------------------
  ;; Will revert if sent any Vs. We use the macro immediately so as
  ;;   to abort if sent any Vsduring contract deployment.
  ...
  (returnlll
    (seq not-payable uses-functions
      ...
      BOOST_AUTO_TEST_CASE(transfer_from)
      {
        ...
        // Send account(1) some vs for gas.
        sendEther(account(1), 1000 * vs);
        BOOST_REQUIRE(balanceAt(account(1)) >= 1000 * vs);
